	...
	<div id="wrap">
		<c:forEach items="${member.vipcards }" var="card">
			<c:if test="${card.status eq '1' }">
				<c:set var="cardNum" value="${card.cardNum }"></c:set>
			</c:if>
		</c:forEach>
  				...
  				<div class="col-xs-6 col-md-6">
  					<div class="form-group">
    					...
    					<input type="text" class="form-control" datatype="m"
    					 nullmsg="手机号码必填" errormsg="请填写正确的手机号码" id="mobile" name="mobile" placeholder="请输入手机号码" value="${member.mobile }">
    					<div class="info"><span class="Validform_checktip Validform_wrong"></span><span class="dec"><s class="dec1">&#9670;</s><s class="dec2">&#9670;</s></span></div>
  					</div>
  				</div>	
			</div>
				...
				<div class="col-xs-6 col-md-6">
  					<div class="form-group">
  						...
  						<div style="height:34px;padding:6px 12px;">
  							<input type="radio" name="sex" <c:if test="${member.sex eq '0' }">checked</c:if> checked value="0"> 男
  				...
  				<div class="col-xs-6 col-md-6">
  					<div class="form-group" >
    					...
    					<select class="form-control" name="memberLevel" id="memberLevel">
  							<option value="1" <c:if test="${member.memberLevel eq '1' }">selected</c:if>>普通会员</option>
  							<option value="2" <c:if test="${member.memberLevel eq '2' }">selected</c:if>>钻石会员</option>
  							<option value="3" <c:if test="${member.memberLevel eq '3' }">selected</c:if>>皇冠会员</option>
						</select>
  					</div>
  				</div>
  				<div class="col-xs-6 col-md-6">
  			...
  			<div class="row">
  				<div class="col-xs-12 col-md-12">
  					<div class="form-group">
    					...
    					<input type="text" value="${member.address }" class="form-control" id="address" name="address" placeholder="请输入详细地址">
  					</div>
  				</div>	
